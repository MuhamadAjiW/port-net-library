...
struct lzmq_interface_t global_lzmq_conn;

// _TODO: use environment variable files
char* global_zmq_server_addr = "127.0.0.1";
int global_zmq_server_port = 8888;
...
uint8_t global_init() {
    ...
    // lzmq_int_init(&global_lzmq_conn, global_zmq_server_addr, global_zmq_server_port, ZMQ_PUB);
    return 1;
}
...
uint8_t global_clean() {
    ...
    // lzmq_int_cleanup(&global_lzmq_conn);
    return 1;
}
